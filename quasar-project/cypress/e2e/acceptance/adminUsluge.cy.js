describe('PT-10: Upravljanje uslugama (admin)', () => {
  beforeEach(() => {
  ...
  it('Administrator dodaje novu uslugu', () => {
    ...
    cy.get('input[aria-label="Naziv"]').type('Svadba')
    cy.get('input[aria-label="Opis"]').type('Opis svadbe')
    cy.get('input[aria-label="Cijena"]').type('100')
    cy.get('input[aria-label="Trajanje (HH:MM:SS)"]').type('04:00:00')
    cy.contains('button', 'Dodaj').click()
    cy.get('.q-table').should('contain', 'Svadba')
  })
